...
const [isCorrect, setIsCorrect] = useState(null); // null, true, or false
  ...
  const handleSaveNotes = async () => {
    const content = scratchpadRef.current?.value;
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.text(`Research Notes: ${problem.title}`, 10, 20);
      ...
      // Split text to wrap within PDF width
      const splitText = doc.splitTextToSize(content, 180);
      doc.text(splitText, 10, 45);
  
      // Convert PDF to Blob
      const pdfBlob = doc.output('blob');
      const fileName = `Draft_${problem.title.replace(/\s+/g, '_')}_${Date.now()}.pdf`;
      ...
      if (saveRes.ok) {
        setLastSaved(new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }));
        fetchAttachments(); // Refresh the list in the Attempts tab
      }
    } catch (error) {
      console.error("Save draft failed:", error);
      alert("Failed to save draft as PDF.");
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="flex-1 bg-[#020617] flex flex-col h-screen overflow-hidden text-slate-300">
      ...
      <nav className="h-14 border-b border-white/5 bg-black/20 backdrop-blur-xl sticky top-0 z-10 flex items-center justify-between px-6">
        ...
        <div className="flex bg-white/5 p-1 rounded-xl border border-white/5">
          {['Workspace', 'Solution', 'History'].map((t) => (
        ...
        {activeTab === 'Workspace' && (
          <div className="space-y-8 animate-in fade-in duration-700">
            <div className="bg-black/40 border border-white/10 rounded-3xl overflow-hidden focus-within:border-sky-500/50 transition-colors shadow-2xl relative">
              ...
              <textarea 
  ref={scratchpadRef} // Add this ref
