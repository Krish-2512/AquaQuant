...
import { 
  ...
  CheckCircle2, Star, Award,Instagram, Mail, MessageCircle, Phone
} from 'lucide-react';
...
const drillQuestions = [
  { id: "QP-PROB-082", q: "At least one of two dice is a 6. What is the probability the sum is 8?", a: "2/11" },
  { id: "QP-PROB-045", q: "What is the expected number of flips to get two consecutive heads?", a: "6" },
  { id: "QP-STAT-012", q: "If Cov(X,Y) = 0, are X and Y always independent? (Yes/No)", a: "No" },
  { id: "QP-PROB-091", q: "A fair coin is tossed 10 times. Probability of exactly 5 heads?", a: "63/256" },
  { id: "QP-PUZ-005", q: "You have 8 identical-looking balls. One is heavier. Min weighings to find it?", a: "2" },
  { id: "QP-FIN-022", q: "A stock is at $100. It goes up 10% then down 10%. What is the final price?", a: "99" },
];

// 2. Update your Drill Section inside the component
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex-1">
              {/* <Quote className="text-sky-500/20 mb-6" size={} /> */}
...
export default function Page() {
  ...
  const handleSubmit = () => {
    const isCorrect = answer.trim().toLowerCase() === drillQuestions[currentIdx].a.toLowerCase();
    
    if (isCorrect) {
      setFeedback({ msg: "CORRECT! LOADING NEXT...", color: "text-emerald-300" });
      setTimeout(() => {
        setFeedback({ msg: "", color: "" });
        setAnswer("");
        setCurrentIdx((prev) => (prev + 1) % drillQuestions.length);
      }, 2000);
    } else {
      setFeedback({ msg: "INCORRECT. TRY AGAIN.", color: "text-red-300" });
  ...
  <div className="relative z-10 w-full mt-10">
    <QuantNexus />
      ...
      <div className="max-w-4xl mx-auto bg-gradient-to-r from-sky-400 to-blue-600 p-16 rounded-[60px] text-center shadow-2xl relative overflow-hidden">
        ...
        <h2 className="text-4xl font-black mb-4 uppercase italic tracking-tight text-white">
          Quant Interview Drill
        </h2>
        <p className="text-sky-100 text-[10px] font-mono uppercase tracking-[0.5em] mb-10">
          Problem_ID: {drillQuestions[currentIdx].id}
        </p>
        ...
        <div className="flex flex-col gap-6 items-center">
          <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md">
            <input 
              ...
              className="bg-black/20 border border-white/20 rounded-2xl px-8 py-5 flex-1 font-mono text-white placeholder:text-sky-200 outline-none focus:border-white transition-all" 
              placeholder="Enter answer..." 
            />
            <button 
              onClick={handleSubmit}
              className="bg-white text-sky-950 px-10 py-5 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-sky-100 transition-colors shadow-2xl"
            >
              Submit Answer
            </button>
          </div>
          ...
          {/* Feedback Message */}
          <div className={`font-mono text-sm font-black tracking-[0.2em] h-6 transition-all duration-300 ${feedback.color}`}>
        {feedback.msg}
      </div>
...
<div className="py-20 border-b border-white/5 text-white">
  <div className="max-w-7xl mx-auto px-6 text-center">
    <span className="text-sky-400 font-mono text-[15px] font-black tracking-[0.4em] uppercase mb-12 block opacity-50">
      Include Questions Asked In
    </span>
    <div className="flex flex-wrap justify-center items-center gap-x-12 gap-y-12 md:gap-x-20 opacity-40 grayscale hover:grayscale-0 transition-all duration-700">
      {['CITADEL', 'JANE STREET', 'OPTIVER', 'HRT', 'IMC TRADING', 'D.E. SHAW', 'TOWER RESEARCH', 'JUMP TRADING', 'SIG', 'TWO SIGMA'].map(firm => (
        <Link 
          ...
          className="contents"
        >
          <span className="text-lg md:text-3xl font-black font-mono text-white tracking-tighter hover:text-sky-400 transition-colors cursor-pointer">
            {firm}
          </span>
        </Link>
      ))}
  ...
  <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-4 gap-16 py-32 text-left text-white">
    <div className="col-span-2">
      ...
      <div className="mt-10">
        <h4 className="text-sky-400/70 font-mono text-[11px] uppercase mb-4 font-bold tracking-[0.2em]">CONTACT US AT</h4>
  ...
  <ul className="text-slate-500 text-sm space-y-4 font-bold uppercase italic tracking-widest">
    {/* THEORY -> # */}
    <li>
      <Link href={session ? "/theory" : "/auth/signin"}  className="hover:text-white cursor-pointer transition-colors block">
        Theory
      </Link>
    </li>

    {/* VAULT -> NOTEBOOK (Session Protected) */}
    <li>
      <Link 
        href={session ? "/notebooks" : "/auth/signin"} 
    ...
    {/* SOLVER -> DASHBOARD (Session Protected) */}
    <li>
      <Link 
        href={session ? "/dashboard" : "/auth/signin"} 
        className="hover:text-white cursor-pointer transition-colors block"
